...
### Functions
1. `handleDragOver`: Prevents the default behavior when an item is being dragged over the drop zone.
...
- If no files are selected, it displays a message to drag and drop files or click to select.
...
In this example, the `FileDropZone` component will be rendered within the `App` component, allowing users to drag and drop files for upload.
